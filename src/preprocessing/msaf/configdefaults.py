...
AddConfigVar('sample_rate', "Default Sample Rate to be used.", IntParam(1))
AddConfigVar('n_fft', "FFT size", IntParam(4096))
AddConfigVar('hop_size', "Hop length in samples", IntParam(1))
...
AddConfigVar('cqt.epsilon',
             "A small number added to the results to avoid divide by zero "
             "errors.",
             FloatParam(1e-8))
...
AddConfigVar('mfcc.ref_power',
             "Reference function to use for the logarithm power.",
             EnumStr("max", "min", "median"))
AddConfigVar('mfcc.mode',
             "Mode for flattening multi-track piano-rolls.",
             EnumStr("stack", "merge"))
AddConfigVar('mfcc.norm',
             "True to normalize the piano-rolls. False to do nothing.",
             BoolParam(True))
AddConfigVar('mfcc.epsilon',
             "A small number added to the results to avoid divide by zero "
             "errors.",
             FloatParam(1e-8))
